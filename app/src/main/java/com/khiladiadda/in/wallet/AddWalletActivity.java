...
//        if (mCoins > 5000 && mAppPreference.getProfileData().getAadharUpdated() != 3) {
        if (mAppPreference.getProfileData().getAadharUpdated() != 3) {
            ...
            if (mPaymentFrom == AppConstant.FROM_PAYTM) {
                ...
                mPresenter.getEasebuzzHash(mEaseBuzzAmount);
            }
        }
    }
    ...
    public void onPaytmPaymentComplete(BaseResponse responseModel) {
        onAllPaymentCompleteCheck(responseModel.isStatus(), responseModel.getMessage());
    }
    ...
    public void onApexPayComplete(ApexPayChecksumResponse responseModel) {
        ...
        if (responseModel.isStatus()) {
            ...
            onPaymentFailed(2, responseModel.getMessage());
        }
    }
    ...
    private void onAllPaymentCompleteCheck(boolean status, String message) {
        hideProgress();
        if (status) {
            onPaymentSuccess();
        } else {
            onPaymentFailed(1, message);
        }
    }
    ...
    private void onPaymentSuccess(){
        ...
        eventParameters.put(AFInAppEventParameterName.REVENUE,mCoins);// Amount of the top-up
        eventParameters.put(AFInAppEventParameterName.CURRENCY,AppConstant.INR);
        AppsFlyerLib.getInstance().logEvent(getApplicationContext(),"top_up_success",eventParameters);
    }
    private void onPaymentFailed(int from, String message) {
        mCouponLL.setVisibility(View.GONE);
        mCouponAppliedTV.setVisibility(View.GONE);
        if (from == 2) {
        ...
        eventParameters.put(AFInAppEventParameterName.REVENUE,mCoins);// Amount of the top-up
        eventParameters.put(AFInAppEventParameterName.CURRENCY,AppConstant.INR);
        AppsFlyerLib.getInstance().logEvent(getApplicationContext(), "top_up_unsuccessful",eventParameters);
    ...
    private final IOnPaymentListener onPaymentListener = result -> {
        if(result) {
                    ...
                    intent.putExtra(AppConstant.FROM,AppConstant.WALLET);
