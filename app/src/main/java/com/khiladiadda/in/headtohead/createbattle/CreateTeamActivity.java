...
import com.khiladiadda.in.dialogs.interfaces.IOnCreateBattleListener;
import com.khiladiadda.in.headtohead.HTHBattlesActivity;
import com.khiladiadda.in.headtohead.createbattle.adapter.HeadToHeadFragmentPagerAdapter;
import com.khiladiadda.in.headtohead.createbattle.fragment.PlayerFragment;
import com.khiladiadda.in.headtohead.createbattle.fragment.ReviewFragment;
...
import com.khiladiadda.in.network.model.request.PlayerId;
...
//        1 = add, 2 = update, 3 = accept
    ...
    public void onJoinComplete(CreateBattleResponse responseModel) {
        ...
        if (responseModel.isStatus()) {
            ...
            eventParameters.put(AppConstant.CLASH_X_JoinPARTICIPATE,1);//
            eventParameters.put(AFInAppEventParameterName.CURRENCY,AppConstant.INR);
            eventParameters.put(AFInAppEventParameterName.REVENUE,mAmount);
            AppsFlyerLib.getInstance().logEvent(getApplicationContext(),"ClashX JoinComplete",eventParameters);
    ...
    public void onAcceptComplete(CreateBattleResponse responseModel) {
        ...
        if (responseModel.isStatus()) {
            ...
            /*Add apps flyer*/
            Map<String, Object> eventParameters = new HashMap<>();
            eventParameters.put(AppConstant.CLASH_X_PARTICIPATE,2);//
            eventParameters.put(AFInAppEventParameterName.CURRENCY,AppConstant.INR);
            eventParameters.put(AFInAppEventParameterName.REVENUE,mAmount);
            AppsFlyerLib.getInstance().logEvent(getApplicationContext(),"clash_x AcceptComplete",eventParameters);
        } else {
            AppUtilityMethods.showMsg(this, responseModel.getMessage(), true);
        }
    }
