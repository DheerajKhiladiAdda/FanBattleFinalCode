...
import com.khiladiadda.in.battle.BattleHelpActivity;
import com.khiladiadda.in.battle.BattlePointsActivity;
import com.khiladiadda.in.fcm.NotificationActivity;
import com.khiladiadda.in.headtohead.adpater.MyBattlesAdpater;
...
import com.khiladiadda.in.headtohead.createbattle.adapter.AllBattlesAdapter;
...
import com.khiladiadda.in.splash.SplashPresenter;
...
public class HTHBattlesActivity extends BaseActivity implements ICreateBattleView, IHTHBattleView, AllBattlesAdapter.IOnItemChildClickListener, MyBattlesAdpater.IOnItemChildClickListener {
    ...
    private int mBattlePosition, mType, mFromSelectedPlayer;
    @Override
        ...
        switch (v.getId()) {
            ...
            case R.id.rl_create_battle:
                if (new NetworkStatus(this).isInternetOn()) {
                    ...
                    mMatchPresenter.getHTHMatchList(mMatchDetail.getId());
                } else {
                    Snackbar.make(mBackIV, R.string.error_internet, Snackbar.LENGTH_SHORT).show();
                }
                break;
    ...
    private final BroadcastReceiver mHTHNotificationReceiverMatches = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            String data = intent.getStringExtra(AppConstant.FROM);
            if (data.equalsIgnoreCase(AppConstant.HTHMATCHES)) {
                if (getLifecycle().getCurrentState().isAtLeast(Lifecycle.State.RESUMED)) {
                    startActivity(new Intent(HTHBattlesActivity.this, AllMatchesHTHActivity.class));
                }
            }
        }
    };
