...
public class PermissionUtils {
    ...
    public static boolean hasReadStoragePermission(final Activity activity) {
        int hasStoragePermission = ActivityCompat.checkSelfPermission(activity, Manifest.permission.READ_EXTERNAL_STORAGE);
        if (hasStoragePermission != PackageManager.PERMISSION_GRANTED) {
            if (ActivityCompat.shouldShowRequestPermissionRationale(activity, Manifest.permission.READ_EXTERNAL_STORAGE)) {
                showMessageOKCancel(activity, "You need to allow access to external storage", AppConstant.REQUEST_GALLERY);
            }
            ActivityCompat.requestPermissions(activity, new String[]{Manifest.permission.READ_EXTERNAL_STORAGE}, AppConstant.RC_ASK_PERMISSIONS_STORAGE);
            return false;
        }
        return true;
    }
...
//                showMessageOKCancel(activity, "You need to allow access to location.", new DialogInterface.OnClickListener() {
//                    @Override
//                    public void onClick(DialogInterface dialog, int which) {
...
//    }
    ...
    public  static boolean  hasGpsPermission(final Activity activity) {
        ...
        if (hasStoragePermission != PackageManager.PERMISSION_GRANTED) {
            if (ActivityCompat.shouldShowRequestPermissionRationale(activity, Manifest.permission.ACCESS_COARSE_LOCATION)) {
                showMessageOKCancelTwo(activity, "FanBattle uses your location to serve you better. Allow FanBattle to access your location", "GPS", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        ActivityCompat.requestPermissions(activity, new String[]{Manifest.permission.ACCESS_COARSE_LOCATION}, AppConstant.RC_ASK_PERMISSIONS_GPS);
                    }
    ...
    private static void showMessageOKCancelTwo(Activity activity, String message, String form, DialogInterface.OnClickListener okListener) {
        new AlertDialog.Builder(activity).setMessage(message).setPositiveButton("OK", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialogInterface, int i) {
                        ...
                        if (form.equalsIgnoreCase("GPS")) {
                            DialogWithCallBack(activity, "Please allow location permission for the first time.");
                        }
                        else
                            DialogWithCallBack(activity, "Please allow location permission for the first time");
    ...
    /*Call App permission Details in setting*/
    private static Dialog DialogWithCallBack(Activity activity, String message) {
        final Dialog dialog = new Dialog(activity);
        dialog.getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
        dialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
        dialog.setContentView(R.layout.popup);
        TextView mMessageTV = dialog.findViewById(R.id.tv_msg);
        mMessageTV.setText(message);
        Button mOkayTV = dialog.findViewById(R.id.btn_ok);
