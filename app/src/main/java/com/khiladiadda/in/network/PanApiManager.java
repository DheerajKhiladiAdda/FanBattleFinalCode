 ...
 */
public class PanApiManager {
    private static PanApiManager sInstance;
     ...
     */
    public static PanApiManager getInstance() {
        if (sInstance == null) {
            synchronized (PanApiManager.class) {
                sInstance = new PanApiManager();
            }
        }
        return sInstance;
    }
    ...
    private Retrofit getRetrofit() {
        ...
        return new Retrofit.Builder().baseUrl(AppConstant.PAN_URL).client(getHttpClient()).addConverterFactory(GsonConverterFactory.create()).addCallAdapterFactory(RxJavaCallAdapterFactory.create()).build();
    }
    ...
    private void initSSL(Context context, OkHttpClient.Builder httpClientBuilder) {
        ...
        try {
            sslContext = createCertificate(context.getResources().openRawResource(R.raw.qa));
