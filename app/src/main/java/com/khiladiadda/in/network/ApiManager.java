...
import com.khiladiadda.in.KhiladiAddaApp;
...
public class ApiManager {
     ...
     * @return the retrofit object use to call the API
     */
    private Retrofit getMultipartRetrofit() {
        Gson gson = new GsonBuilder().registerTypeAdapter(Date.class, UnixEpochDateTypeAdapter.getUnixEpochDateTypeAdapter()).create();
        return new Retrofit.Builder().baseUrl(AppUtilityMethods.getURL(AppConstant.URL)).client(getMultipartHttpClient()).addConverterFactory(GsonConverterFactory.create()).addCallAdapterFactory(RxJavaCallAdapterFactory.create()).build();
    }
     ...
     * @return the ApiService instance
     */
    public ApiService createMultipartService() {
        Retrofit retrofit = getMultipartRetrofit();
        return retrofit.create(ApiService.class);
    }
     ...
     */
    private OkHttpClient getMultipartHttpClient() {
        ...
        httpClient.addInterceptor(chain -> {
            ...
            try {
                request = original.newBuilder().
                        ...
                        header(AppConstant.TEXT_CONTENT_TYPE, AppConstant.TEXT_CONTENT_TYPE_MULTIPART).
    ...
    private void initSSL(Context context, OkHttpClient.Builder httpClientBuilder) {
        ...
        try {
            sslContext = createCertificate(context.getResources().openRawResource(R.raw.prodps));
